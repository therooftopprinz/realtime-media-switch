...
class JsGenerator:
    ...
    def emit_runtime_import(self):
        print(
            ...
            "writeIntegralLE, readIntegralLE } "
            'from "../cum/cum.mjs";'
        )
        print("")

    def emit_packed_primitive_helpers(self):
        """u8/u16/u32/u64 and Latin-1 C strings — required for sequence fields (match target_py3)."""
        print("// Fixed-width scalars (LE) + C strings")
        for n, nbytes in (("u8", 1), ("u16", 2), ("u32", 4), ("u64", 8)):
            print("export function encodeUsing_{}(v, ctx) {{".format(n))
            if nbytes == 1:
                print("    ctx.writeU8(Number(v) & 0xff);")
            else:
                mod = 1 << (8 * nbytes)
                print(
                    "    const vv = Number(BigInt.asUintN({}, BigInt(v)));".format(nbytes * 8)
                )
                print("    if (ctx.remaining() < {}) throw new CodecError('encode buffer full');".format(nbytes))
                print("    writeIntegralLE(ctx.buf, ctx.off, vv, {});".format(nbytes))
                print("    ctx.off += {};".format(nbytes))
            print("}")
            print("")
            print("export function decodeUsing_{}(ctx) {{".format(n))
            if nbytes == 1:
                print("    return ctx.readU8();")
            else:
                print("    if (ctx.remaining() < {}) throw new CodecError('decode overrun');".format(nbytes))
                print(
                    "    const v = readIntegralLE(ctx.buf, ctx.off, {});".format(nbytes)
                )
                print("    ctx.off += {};".format(nbytes))
                print("    return v;")
            print("}")
        ...
        print("export function encodeUsing_string(v, ctx) {")
        print("    ctx.encodeCStringLatin1(v);")
        print("}")
        print("")
        print("export function decodeUsing_string(ctx) {")
    ...
    def emit_using_codec(self, alias):
        ...
        arrlen = td.get("array")
        if arrlen is not None:
            nfix = self._cum_int_literal(arrlen)
            elem_inner = td["type"]
            if elem_inner == "u8":
                print("export function encodeUsing_{}(arr, ctx) {{".format(alias))
                print(
                    "    if (arr.length !== {n}) throw new CodecError('{a} length');".format(
                        n=nfix, a=alias
                    )
                )
                print("    ctx.writeCount({n}, arr.length);".format(n=nfix))
                print(
                ...
                print("")
                print("export function decodeUsing_{}(ctx) {{".format(alias))
                print("    const n = ctx.readCount({});".format(nfix))
                print("    const arr = [];")
                print("    for (let k = 0; k < n; k++) arr.push(decodeUsing_u8(ctx));")
                print("    return arr;")
                print("}")
                print("")
                return
            raise RuntimeError(
                "static arrays are not emitted for {} (element {!r})".format(
                    alias, elem_inner
                )
            )
    ...
    def _encode_one_field_ordered(self, tname, fname):
        ...
        if tname == "string":
            print("    encodeUsing_string({}, ctx);".format(fa))
            return
    ...
    def emit_sequence_codec(self, name):
        ...
        for tname, fname in flds:
            ...
            if tname in self.type_ and self.type_[tname].get("optional"):
                ...
                continue
            if tname == "string":
                print("    {} = decodeUsing_string(ctx);".format(fa))
            elif tname in self._PRIMITIVE_FIELDS:
    ...
    def generate(self):
        ...
        self.emit_packed_primitive_helpers()
